#!/system/bin/sh
# 01-bind-hosts.sh: bind-mount hosts overlay (legacy)
...
if [ -f "$HOSTS_FILE" ]; then
    ...
    if [ -z "$MOUNTED" ]; then
        ...
        log "[01-bind-hosts] hosts overlay bind-mount activated"
    else
        log "[01-bind-hosts] hosts overlay already activated"
    fi
else
    log "[01-bind-hosts] hosts file not found!"
fi